function plot_OutVSInp_DiatCoco_NOBM
...
yl = [0.9 0.9]; % ylimits corresponding to these areas
...
for i = 1 : 2 ; % species loop
    ...
    for j = 1 : 5 ; % indicator loop
        ...
        for g = 1 : 8 ; % Run the loop for N forecast domains
            ...
            %p1 = 0.0 * ( 1:sI ) ; % areas
            if j == 4 || j == 5;
                p1 (g) = scatter ( squeeze(10.^(forecast.inputs(j,:))), forecast.targets ( i, : ) ,3 ,clr{g} ,'o' ,'filled' ) ;
                set( gca,'XScale','log','XTick',[0.01 0.1 1 10 100],'XTickLabel',{'0.01','0.1','1','10','100'});
                %set( gca, 'XTickLabel', num2str(get(gca,'XTick')','%d'))
            else
                p1 (g) = scatter ( squeeze(forecast.inputs(j,:)), forecast.targets ( i, : ) ,3 ,clr{g} ,'o' ,'filled' ) ;
            end;
...
%% Save figure
filename = [ puboutdir, 'f-diatcoco-vs-indicators-NOBM' ] ;
