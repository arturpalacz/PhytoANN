function plot_OutVSInp_CyanChloro_PhytoANN (InDir, netDate, Params, TraParams, ForParams)
...
switch store
    case 1
        figOutDir = '/media/arpa/TOSHIBA EXT/Results/PhytoANN/plots/'; % for result viewing
...
yl1 = [0.12 0.5]; % ylimits corresponding to these areas
...
for i = 1 : 2 ; % species loop
    ...
    for j = 1 : 5 ; % indicator loop
        ...
        for g = 1 : 8 ; % Run the loop for N forecast domains
                ...
                case 1
                    if (j-1)+j+(i-1) >= 9; % only for the last two panels do that
                        for m = 1 : sP ;
                            ...
                            %NOBMnorm(m,:) = forecast.targets(m,:) ./ sum ( forecast.targets, 1 ) * 100 ;
                        end;
                    else
                        phytoPFTnorm = forecast.AvgOutput;
                    end;
                case 2
                    phytoPFTnorm = forecast.AvgOutput ;
            end;
            ...
            %p1 = 0.0 * ( 1:sI ) ; % areas
            if j == 4 || j == 5;
                p1 (g) = scatter ( squeeze(10.^(forecast.inputs(j,:))), phytoPFTnorm ( i+2, : ) ,3 ,clr{g} ,'o' ,'filled' ) ;
                set( gca,'XScale','log','XTick',[0.01 0.1 1 10 100],'XTickLabel',{'0.01','0.1','1','10','100'});
                %set( gca, 'XTickLabel', num2str(get(gca,'XTick')','%d'))
            else
                p1 (g) = scatter ( squeeze(forecast.inputs(j,:)), phytoPFTnorm ( i+2, : ) ,3 ,clr{g} ,'o' ,'filled' ) ;
            end;
...
%% Save figure
filename = [ figOutDir, 'f-cyanchloro-vs-indicators-ANN-FracChl' ] ;
