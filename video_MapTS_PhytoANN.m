% --------------------------------------------------------------------------------------
  function video_MapTS_PhytoANN ( InDir, netDate, Params, TraParams, ForParams )
% --------------------------------------------------------------------------------------
...
switch store
    ...
    case 2
        % figOutDir = '/home/arpa/Dropbox/KileProjects/paper-Aqua-PhytoBiogeo/figures/'; % for publication submission
          vidOutDir = '/home/arpa/Dropbox/KileProjects/paper-Aqua-PhytoBiogeo/animations/'; % for publication submission
end;
...
if strcmp (ForParams.TarSource,'medusa') == 1;
    species = {'diatoms','NA','NA','non-diatoms';
               'diatoms','coccos','cyanos','chlorophytes'};
elseif strcmp (ForParams.TarSource,'nobm') == 1;
    species = {'diatoms','coccos','cyanos','chlorophytes';
               'diatoms','coccos','cyanos','chlorophytes'};
end;
...
[ ForParams.Geo ] = ask_domain_PhytoANN ( 'forecast' );
...
Contours = [.001 .01 .02 .03 .05 .1 .2 .3]; % Contour levels in units of mg-Chl m-3
...
vidObj = VideoWriter([vidOutDir,'PhytoANN_vs_',ForParams.TarSource,'_',ForParams.Geo.Basin,'_',ForParams.Time.TyStart,'-',ForParams.Time.TyEnd,'_animation.avi']);
...
set(Figure1, 'color'   , 'w'  , ...
    'Visible' , 'off' ,...
    'Units', 'centimeters',...
    'Position', [2 2 20 20] ,... % 20 cm x 10 cm - make this in agreement with publication requirements
    'renderer', 'painters' );
...
% ----------------------------------------------------------------------
    subaxis ( 2, sP, i, 'Spacing', 0.1, 'Padding', 0, 'Margin', 0.07)
% ----------------------------------------------------------------------
        ...
        title ( [ForParams.TarSource, ' ', spcs{1,i}], 'FontSize', fs-2 ) ;
    
% ----------------------------------------------------------------------
    subaxis ( 2, sP, sP+i, 'Spacing', 0.1, 'Padding', 0, 'Margin', 0.07)
% ----------------------------------------------------------------------
    ...
    title ( ['PhytoANN ', spcs{2,i}], 'FontSize', fs-2 ) ;
        
    end;

    ax = axes('position',[0,0,1,1],'visible','off');
    tx1 = text(0.44,0.95,num2str(datestr(ForTime(t),'mmm-yyyy')),'FontWeight','bold','FontSize',fs+4);
    
    ax = axes('position',[0,0,1,1],'visible','off');
    tx2 = text(0.26,0.54,'.01','FontSize',fs);
    
    ax = axes('position',[0,0,1,1],'visible','off');
    tx3 = text(0.45,0.54,'mg-Chl m^{-3}','FontSize',fs);
   
    ax = axes('position',[0,0,1,1],'visible','off');
    tx4 = text(0.73,0.54,'> 1.5','FontSize',fs);
    ...
    pos = get(gca,'pos');
    hc = colorbar('location','northoutside','position',[pos(1)+.25 pos(2)+.50 pos(3)-.5 0.02]);
